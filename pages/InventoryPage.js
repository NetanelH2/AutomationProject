...
export class InventoryPage {
  ...
  async assertOnPage() {
    await this.productsTitle.waitFor({state: 'visible'}) // You don't need this method, its only necessary so special situation related to a page taking a lot of time to load
  }
   ...
   */
  async addFirstNItems(n) { // why using this kind of loop? use for ... of loop instead, its more readable and less error prone
    for (let i = 0; i < n; i++) {
  ...
  async assertCartCount(expectedCount) {
    await this.cartBadge.waitFor({state: 'visible'}) // You don't need this method, its only necessary so special situation related to a page taking a lot of time to load
    const badgeText = await this.cartBadge.innerText()
    const actual = Number(badgeText)

    if (actual !== expectedCount) {
      throw new Error(
        `Expected cart count ${expectedCount} but got ${badgeText}`,
      )
    }
  }
